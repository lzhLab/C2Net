...
def attention(img_tensor, model=None, direction='W'):
    ...
    img_vol = F.pad(img_tensor, (0, pad_w, 0, pad_h), mode='constant', value=0) #8,3,258,258
    ...
    if direction == 'H':
        for b in range(B):
            for h in range(0, H, 3):
                img_chunk = img_vol[b, :, h: h + 3, :].transpose(0, 2) #[3,3,258]->[258,3,3]
                inputs_lst.append(img_chunk.reshape(-1, 27)) #[86,27]->668[86,27]
    elif direction == 'W':
        for b in range(B):
    ...
    inputs = torch.stack(inputs_lst) #[688,86,27]
    model.eval()
    with torch.no_grad():
        preds = torch.sigmoid(model(inputs)).squeeze() #[688,86,1]->[688,86]

    step = preds.shape[0] // B
    lst = [preds[i: i + step] for i in range(0, preds.shape[0], step)] #8*[86,86]
    preds = torch.stack(lst) #[8,86,86]

    if direction == 'W':
        preds = preds.transpose(1, 2)

    preds = F.interpolate(preds.unsqueeze(1), size=[H, W], mode='nearest') #[8,1,256,256]
    # print(preds.shape)
    return preds
...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in alive_it(range(1, self.args.epochs + 1)):
            ...
            if (epoch + 1) % 2 == 0: #每两个保存一次模型参数，保存模型torch.save(model, model_path)
            ...
            if best_val_dice < aver_val_dice: #保存最好的模型参数
    ...
    def val_epoch(self):
        ...
        with torch.no_grad():
            ...
            for liver_imgs, tumor_mask in self.valloader:
                ...
                if self.rnn != None:
                    att_map_h = attention(liver_imgs, self.rnn, 'H') #[8,1,256,256]
                    att_map_w = attention(liver_imgs, self.rnn, 'W')
                    merge = torch.cat([att_map_h, att_map_w], dim=1)
                    att_map, _ = torch.max(merge, dim=1, keepdim=True)
